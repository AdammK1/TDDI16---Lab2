...
Key::Key(const char *value) : digit{} {
    for (int i{0}; i < C; i++) {            //Ittererar igenom strängen ex asd.
        for (int j{0}; j < R; j++) {        //Ittererar igenom alfabetet. A - Z samt 0-5-
            if (value[i] == ALPHABET[j])    // Om karaktären finns i alfabetet. 
                digit[i] = j;               // Tilldela digit(privat datamedlem (char)) värdet i alfabetet.
        }
    }
}
